...
export class NginxPipelineStack extends Stack {
  ...
  const registry = new ecr.Repository(this,`nginx-servers`, {
  ...
  const buildproject = new codebuild.Project(this, `NginxServerDockerBuild`, {
       environment: {
            privileged: true,
      ...
      buildSpec: codebuild.BuildSpec.fromObject({
        ...
        phases: {
          build: {
            commands: [`TAG=$(date +'%Y%m%d%H%M%S')`,`echo $TAG`,`docker build -t ${this.account}.dkr.ecr.${this.region}.amazonaws.com/${registry.repositoryName}:${baseImageVersion.valueAsString} .`,
