...
while true
do
  is_connect_to_server=`cat $game_stdout | grep "ConnectToServer: Connected to $ENDPOINT"|wc -l`
  ...
  if (( $is_connect_to_server > 0 )); then
    is_sending_packet_started=`tail $game_stdout | grep "STKPeer: sending packet of size" | grep "$ENDPOINT at" | wc -l`
    if (( $is_sending_packet_started > 0 )); then
      echo "connected to server and sending packets"
      break
    fi
  else
